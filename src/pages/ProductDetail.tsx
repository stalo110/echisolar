      ...
      <Container sx={{ py: 6 }}>
        <Grid container spacing={4}>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            ...
            <Typography color="text.secondary" sx={{ my: 2 }}>{product.description}</Typography>
            <Typography variant="h6" sx={{ fontWeight: 'bold' }}>NGN {product.price.toLocaleString()}</Typography>
            ...
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', mt: 2 }}>
              ...
              <Button variant="contained" color="success" onClick={()=> add({ productId: product.id, name: product.name, price: product.price, quantity: qty })}>Add to Cart</Button>
              <Button variant="outlined" onClick={()=> { clear(); add({ productId: product.id, name: product.name, price: product.price, quantity: qty }); navigate('/checkout'); }}>Buy Now</Button>
            </Box>
          </Grid>
        </Grid>
        ...
        {related.length > 0 && (
          <Box sx={{ mt: 6 }}>
            <Typography variant="h5" sx={{ fontWeight: 'bold', mb:2 }}>Related Products</Typography>
                ...
                <Grid size={{ xs: 12, sm: 6, md: 3 }} key={r.id}>
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <Typography color="text.secondary">NGN {r.price.toLocaleString()}</Typography>
                    </CardContent>
                  </Card>
                </Grid>
              ))}
